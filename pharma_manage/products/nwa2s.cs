...
{
    public partial class nwa2s : Form
    {
        public nwa2s()
        {
            InitializeComponent();
            ...
            else
            {
                ...
                if (sf.ShowDialog() == DialogResult.OK)
                {
                    ...
                    foreach (DataGridViewRow dgr in dataGridView1.Rows)
                    {
                        if (Convert.ToBoolean(dgr.Cells[6].Value) == true)
                        {
                            nwa2esTableAdapter1.Insert(dgr.Cells[0].Value.ToString(), dgr.Cells[3].Value.ToString(), dgr.Cells[2].Value.ToString(), Convert.ToDecimal(dgr.Cells[4].Value), Convert.ToDecimal(dgr.Cells[5].Value), dgr.Cells[1].Value.ToString());
                        }
                    }
                    ...
                    se3r_offer1.ExportToDisk(CrystalDecisions.Shared.ExportFormatType.PortableDocFormat, path_file.ToString());
        ...
        private void nwa2s_Load(object sender, EventArgs e)
        {
            ...
            if (this.Text.Contains("نواقص"))
            {
                if (this.Text.Contains("الكل"))
                {
                    far3_txt.Visible = false;
        ...
        {
            dataGridView1.Rows.Clear();
            this.products_tableTableAdapter1.FillBy_stock(this.dataSet11.products_table,far3_txt.Text);
            int i = 0;
            //MessageBox.Show("جرب من بره البرنامج");
            foreach (DataRow dr in dataSet11.products_table.Rows)
            {
